...
def fitTime(Opt,begin,end,step):
	...
	Y.append(Double1dTimeCost.TimeCal(Opt,i))
    Y=Double1d(Y)
    ...
    myPlot.titleText = "Time Used of the Double1d " + Opt +"Operation"
    myPlot.xaxis.titleText = "number of elements in one dimension"
...
def TimeUsed(Opt,Path,begin,end,step):
    ...
    myData=Double1dTimeCost.TcInput(Opt,Path)
    if(len(myData[0])==0):
	return 0
...
myPlot.titleText = "real time used of  the pipeline" 
