...
# 
###########################################################################
###            SPIRE Parallel Mode User Reprocessing Script            ###
###########################################################################
#  Purpose:  A simplified version of SPIRE Parallel Mode  pipeline   
...
#
myObsid    =  1342186275
myDataPool = "1342186275"
outDir     = "C:\\Users\\yfjin\\hcss\\fitresult\\"
# e.g.
#myObsid    =  0x50002E17
#myDataPool = "OD239-ParallelM33-0x50002E17"
...
# (I) coolerBurpDetection: Search and correct cooler burp recalculating the TempDrift calibration table
mapping = 'naive'
includeTurnaround = False
applyExtendedEmissionGains = False
useBaselineSubtraction = False
useDestriper = True
coolerBurpDetection = False
###########################################################################
...
obs=getObservation(myObsid,poolName=myDataPool,instrument="SPIRE")
...
for bbid in bbids:
    ...
    #            largeGlitchDiscriminatorTimeConstant = 3, \
    ...
    #
    # -----------------------------------------------------------
    # (10) Check for cooler burp
    coolerBurpFound = False		    
    if coolerBurpDetection :
		threshold=3.e-7
		tmpTime= nhkt.getSampleTime()
		tmpSubk = nhkt.getSignalData("SUBKTEMP")
		if (tmpSubk[-1]-tmpSubk[0])/(tmpTime[-1]-tmpTime[0]) > threshold:
			coolerBurpFound=Boolean.TRUE
		pass
    pass
    if coolerBurpFound:
		print "Cooler burp detected!"
		newTempDrift=recalcTempDrift(pdt,tempDriftCorr)
    else:
		newTempDrift=tempDriftCorr
    pass
    # -----------------------------------------------------------
    # (11) Make the temperature drift correction
    pdt=temperatureDriftCorrection(pdt,tempDriftCorr=newTempDrift)
    #
    # -----------------------------------------------------------
